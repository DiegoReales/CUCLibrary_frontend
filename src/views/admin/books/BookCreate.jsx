...
const BookCreate = ({ handleRefresh }) => {
  ...
  const onSuccess = () => {
    return toast.success('Libro creado con éxito!', {
      style: {
        padding: '16px',
  ...
  const onFail = () => {
    return toast.error('Ha ocurrido un error al crear libro!', {
      style: {
        padding: '16px',
  ...
  const handleCreate = async book => {
    ...
    try {
      await bookService.bookCreate(book)
      onSuccess()
      handleRefresh()
      setIsLoading(false)
      setShow(false)
  ...
  return (
    <>
      <Button onClick={() => handleClick()} variant='contained'>
        <IconifyIcon icon='tabler:circle-plus' />
        <span> Nuevo</span>
      </Button>
      <Dialog
        fullWidth
