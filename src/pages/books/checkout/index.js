...
const CheckOut = () => {
  ...
  const getBooks = () => bookService.getBooksAvailable().then(b => setBooks(b))
  ...
  const checkOutSuccess = () => {
    return toast.success('Libro prestado con éxito!', {
      style: {
        padding: '16px',
  ...
  const checkOutFail = () => {
    return toast.error('Ha ocurrido un error al prestar libro!', {
      style: {
        padding: '16px',
  ...
  const handleCheckOut = b => async () => {
    setIsLoading(true)

    try {
      await bookService.bookCheckOut(b.id)
      getBooks()
      checkOutSuccess()
      setIsLoading(false)
      setShow(false)
          ...
          {selectedBook && (
            ...
            >
              <Button
                ...
                sx={{ mt: 4 }}
                disabled={isLoading}
                onClick={handleCheckOut(selectedBook)}
              >
                {isLoading ? <CircularProgress color='inherit' /> : <span>Confirmar Presatamo</span>}
              </Button>
            </CardBook>
          )}
        </DialogContent>
